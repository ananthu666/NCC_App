...
// console.log("month_id",month_id);
  ...
  const fetch_cred =async()=>{
    try {
        ...
        const q = query(collection(database, "unit_credit"), where("month_id", "==", month_id));
...
const fetch_debit =async()=>{
    try {
        ...
        const q = query(collection(database, "unit_debit"), where("month_id", "==", month_id));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
          const debitsData = [];
          querySnapshot.forEach((doc) => {
            const cadet = {
  ...
  const [activeComponent, setActiveComponent] = useState('Finance_cred'); // State to determine which finance component to show
    ...
    const renderFinanceComponent = () => {
        ...
        switch (activeComponent) {
            case 'Finance_cred':
                return <Finance_cred  camp_id={month_id} data={cred_data} loading={loading}/>;
            case 'Finance_deb':
                return <Finance_deb month_id={month_id} data={debit_data} loading={loading}/>;
    ...
    <div style={styles.cont}>
        ...
        <div style={styles.btnContainer}>
            ...
            <button
                ...
                style={{ ...styles.btn, ...(activeComponent === 'Finance_cred' && styles.activeBtn) }}
            >
                Credit
            </button>
            <button
                onClick={() => toggleComponent('Finance_deb')}
                style={{ ...styles.btn, ...(activeComponent === 'Finance_deb' && styles.activeBtn) }}
            >
                Debit
            </button>
            <button
                onClick={() => toggleComponent('Finance_add_cred')}
                style={{ ...styles.btn, ...(activeComponent === 'Finance_add_cred' && styles.activeBtn) }}
            >
                Add Credit
            </button>
            <button
                onClick={() => toggleComponent('Finance_add_deb')}
                style={{ ...styles.btn, ...(activeComponent === 'Finance_add_deb' && styles.activeBtn) }}
            >
                Add Debit
            </button>
        </div>
